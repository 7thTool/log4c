...
public:
    ...
    int Init(const char *path, const char *name, int flags)
    {
    ...
    {
        if(!(flags_&LOG4C_FLAG_FILE_LEVEL)) {
            level = 0;
        }
        time_t tt;
        time(&tt);
...
int LOG4C_Init(const char *path, const char *name, int flags)
{
    return CLOG4CMgr::Inst().Init(path, name, flags);
}
